...
import numpy as np
import argparse
from utils import decode, gen_poly
...
if __name__ == "__main__":
    ...
    decoded = map(lambda v: gen_poly(v[0], v[1], v[2]), decoded)

    decoded = list(decoded)
    decoded = list(map(lambda polys: [poly.values() for poly in polys], decoded))

    # now decoded is a list of list of (n,2) arrays
    # convert each list to a list of points
    total_points = []
    for points in decoded:
        current = []
        for point in points:
            current.append((point[0].item(), point[1].item()))
        total_points.append(current)

    print(f"Decoded {len(total_points)} entries from {filename}")

    print("Xi shapes:", total_points)

    colors = plt.get_cmap("tab10")(np.linspace(0, 1, len(total_points)))
